...
export function pageClickElement(selector, text) {
  ...
  if (!el && text) {
    ...
    const candidates = Array.from(document.querySelectorAll("button, a, input[type='button'], input[type='submit'], [role='button'], label, .btn, span, td, tr, fluent-button, fluent-anchor, fluent-tree-item, fluent-tab, fluent-checkbox, fluent-switch, fluent-radio"));
...
export function pageTypeText(selector, text, clear) {
  ...
  if (!el) {
    ...
    if (a && (a.tagName === "INPUT" || a.tagName === "TEXTAREA" || a.isContentEditable || a.tagName.startsWith("FLUENT-"))) {
      el = a;
    }
  }
  if (!el) {
...
export async function pageSnapshot(opts) {
  ...
  function rowHasInteractive(tr) {
    return L.queryAllDeep(tr, "a[href], button, input, select, textarea, [role], [onclick], fluent-button, fluent-anchor, fluent-checkbox, fluent-radio").some(x => {
      const rr = L.roleOf(x);
...
export async function pageResolve(query, opts) {
  ...
  function interactiveCandidates() {
    return L.queryAllDeep(document, "a[href], button, input, select, textarea, [role], [onclick], [contenteditable], summary, fluent-button, fluent-anchor, fluent-text-field, fluent-search, fluent-select, fluent-checkbox, fluent-switch, fluent-radio, fluent-tree-item, fluent-tab")
