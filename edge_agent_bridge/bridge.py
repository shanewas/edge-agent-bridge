...
def _heartbeat_worker(state: BridgeState, stop_event: threading.Event, server: "BridgeServer"):
...
class BridgeRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith("/api/tabs"):
            ...
            cmd_payload = {"id": cmd_id, "action": "list_tabs", "params": {}}
            with state.lock:
                if state.ext and not state.ext.closed and state.ext_accepted:
                    ...
                    if not sent:
                        ...
                        state.held.append(HeldCommand(cmd_id, cmd_payload, event, deadline))
                else:
                    state.held.append(HeldCommand(cmd_id, cmd_payload, event, deadline))
            rem = max(0.01, deadline - time.time())
            finished = event.wait(timeout=rem)
            with state.lock:
                res = state.results.pop(cmd_id, None) if finished else None
            ...
            if finished and res is not None:
                ...
                self._respond(504, {"success": False, "code": "timeout", "error": "Timed out waiting for tabs"})
            return
                ...
                if cmd_id in state.pending:
                    ...
                    if is_v1 and ((isinstance(err, str) and err.startswith("Unknown action")) or code == "unknown_action"):
                        ...
                        result = {
                            ...
                            "error": f"Extension {state.ext_version or 'older than 2.0.0'} is connected; daemon 2.0.0 needs extension 2.0.0 or newer. Update it from the Edge Add-ons store, or reload the unpacked extension from {config.extension_dir()}."
                        }
    ...
    def do_POST(self):
        ...
        if self.path.startswith("/api/eval"):
            if self._is_browser_request():
                self._respond(403, {"success": False, "code": "browser_context", "error": "/api/eval is not reachable from browser context"}, cors=False)
                return
            req, err = self._read_body()
            if err is not None or not isinstance(req, dict):
                self._respond(400, {"success": False, "code": "bad_body", "error": err or "Invalid JSON"}, cors=False)
                return
            code = req.get("code")
            if not code:
                self._respond(400, {"success": False, "code": "bad_body", "error": "Missing 'code'"}, cors=False)
                return
            try:
            ...
            if tab_id is not None:
                params["tabId"] = tab_id
            cmd_payload = {"id": cmd_id, "action": "eval", "params": params}
            with state.lock:
                if state.ext and not state.ext.closed and state.ext_accepted:
                    state.pending[cmd_id] = event
                    sent = state.ext.send_text(json.dumps(cmd_payload))
            ...
            if finished and res is not None:
                ...
                self._respond(504, {"success": False, "code": "timeout", "error": "Command timed out"}, cors=False)
            return
...
def run_server(host="127.0.0.1", port=None, home=None, require_pairing=None) -> None:
    ...
    hb_thread = threading.Thread(target=_heartbeat_worker, args=(state, stop_event, server), daemon=True)
    hb_thread.start()
...
def main(argv=None) -> int:
    ...
    elif args.no_pairing:
        pairing = False
        config.write_config({"pairing": "0"})
