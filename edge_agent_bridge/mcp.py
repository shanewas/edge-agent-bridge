...
def handle(msg: dict, edge: Edge) -> dict | None:
    ...
    # Notifications
    if not msg_id or method in ("notifications/initialized", "notifications/cancelled"):
        return None
    ...
    if method == "tools/call":
        ...
        elif tool_name == "edge_screenshot" and not is_error and res.get("data"):
            data_str = res["data"]
            if "," in data_str:
                data_str = data_str.split(",", 1)[1]
            mime_type = "image/png" if args.get("format") == "png" else "image/jpeg"
            content = [
