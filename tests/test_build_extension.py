...
def test_name_and_description_stay_within_store_limits(tmp_path, ext_copy):
    from edge_agent_bridge import __version__
    import sys as _sys
    ...
    assert data["name"] == be.NAME == data["action"]["default_title"]
    ...
    # The listing is vendor-neutral; "Edge" in a Chrome Web Store name is a trademark.
    assert "Edge" not in data["name"]
    assert (tmp_path / "dist" / f"edge-agent-bridge-extension-{__version__}.zip").exists()
